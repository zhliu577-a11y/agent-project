...
def test_loop_ends_when_model_does_not_call_tools() -> None:
    model = FakeModel([ModelResponse(content="你好", tool_calls=[])])
    ctx = run_agent(model, ToolRegistry(), HookManager(), "你是助手", "你好")
...
def test_loop_executes_tool_then_returns_final_answer() -> None:
    ...
    ctx = run_agent(model, tools, HookManager(), "你是助手", "现在几点")
    assert ctx.stop_reason == "done"
...
def test_loop_stops_at_max_turns() -> None:
    ...
    tools = ToolRegistry()
    tools.register(GetTime())
    ctx = run_agent(model, tools, HookManager(), "你是助手", "一直调用", max_turns=3)
