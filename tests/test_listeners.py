...
def test_listener_cannot_subscribe_decision_event(tmp_path) -> None:
    module = """
from core.events import Subscription


def create_listener(plugin_dir):
    return [Subscription(event="user_prompt.submit", handler=lambda event: "deny")]
"""
    _write_listener(tmp_path, module=module)
    plugin = load_listener_plugins(tmp_path)[0]
    with pytest.raises(ValueError, match="决策类事件"):
        plugin.create()
